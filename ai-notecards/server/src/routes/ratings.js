...
router.post('/', requireXHR, authenticate, requireActiveUser, async (req, res) => {
  ...
  // Validate review_text type and length
  ...
  try {
    ...
    try {
      ...
      // If ON CONFLICT hit, the INSERT returned no rows — duplicate rating
      if (insertResult.rows.length === 0) {
        await client.query('ROLLBACK');
        return res.status(409).json({ error: 'You have already rated this listing' });
      }

      // Self-correcting average recalculation (SELECT AVG/COUNT, not incremental)
      await client.query(
