...
const DEFAULT_PURCHASE_PLACEHOLDER = Object.freeze({
  status: 'unavailable',
...
export function createMarketplaceRouter({
  ...
  poolInstance = pool,
  createPurchaseCheckoutFn = createPurchaseCheckout,
  getPurchaseAvailabilityFn = getMarketplacePurchaseAvailability,
  purchasePlaceholder = DEFAULT_PURCHASE_PLACEHOLDER,
} = {}) {
  const router = Router();
  let lastCleanup = 0;
  ...
  router.get('/', async (req, res) => {
    ...
    try {
      ...
      res.json({
        ...
        nextCursor,
        hasMore,
        purchaseAvailability: getPurchaseAvailabilityFn(),
      });
    } catch (err) {
  ...
  router.get('/:id', async (req, res) => {
    try {
      const { rows: listings } = await poolInstance.query(`
      ...
      const { rows: countRows } = await poolInstance.query(
      ...
      const { rows: sampleCards } = await poolInstance.query(
  ...
  router.post('/:id/flag', requireXHRMiddleware, authenticateMiddleware, requireActiveUserMiddleware, async (req, res) => {
  ...
  router.post('/:id/purchase', requireXHRMiddleware, authenticateMiddleware, requireActiveUserMiddleware, async (req, res) => {
    try {
      const purchaseAvailability = getPurchaseAvailabilityFn();
